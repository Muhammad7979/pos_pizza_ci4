...
class Cake_suspended extends Model
{
    protected $table = 'ospos_cake_suspended';
    public function get_all()
    {
       $builder = $this->db->table($this->table)
             ->orderBy('cake_suspended.sale_id');
    ...
    public function get_info($sale_id)
    {
       $builder = $this->db->table('cake_suspended')
                       ...
                       ->join('people', 'people.person_id = cake_suspended.customer_id', 'LEFT');
    ...
    */
    ...
    public function update_cake_suspended($sale_data, $sale_id)
    {
        $this->db->where('sale_id', $sale_id);

        return $this->db->update('cake_suspended', $sale_data);
    }

    public function save_cake_suspended($items, $customer_id, $employee_id, $comment, $invoice_number, $payments, $sale_id = FALSE)
    {
        if (count($items) == 0) {
            return -1;
        }
      ...
      // Through cakeapp 
       public function delete_cake_suspended($sale_id)
    {
        $appConfig = new Appconfig();
        $cake_app_ip = $appConfig->get('cake_app_ip');
        $apiUrl = 'http://'.$cake_app_ip.'/api/suspendedOrderDelete/'.$sale_id;
         ...
         if (isset($data['error'])){
          // Handle error
          echo "There occurs an error in server!-delete";
          return false;
         ...
         $this->db->table('cake_suspended_payments')->delete(['sale_id' => $sale_id]);
         $this->db->table('cake_suspended_items_taxes')->delete(['sale_id' => $sale_id]);
         $this->db->table('cake_suspended_items')->delete(['sale_id' => $sale_id]);
         $this->db->table('cake_suspended')->delete(['sale_id' => $sale_id]);
    ...
    {
       $builder = $this->db->table('cake_suspended_items')
                    ->where('sale_id', $sale_id);

        return $builder->get();
    }

    public function get_sale_items_by_invoice($invoice)
    {
        $this->db->from('cake_suspended');
        $this->db->where('cake_invoice', $invoice);
    ...
    public function get_sale_items_info($sale_id)
    {
        $builder = $this->db->table('cake_suspended_items')
                    ->where('sale_id', $sale_id)
                    ->join('items', 'cake_suspended_items.item_id = items.item_id', 'LEFT');
    ...
    public function search($search){
       ...
       $buider =  $this->db->table('cake_suspended')
                       ->like('cake_invoice', $search);
    ...
    public function update_order_status($cake_invoice){
        ...
        if($response === false){
         ...
         if (isset($data['error'])){
            // Handle error
            echo $data['error'] .' '. "There occurs an error in server!-updateOrderStatus";
            return false;
    ...
    public function get_cake_order_info($item_id)
	{
		$this->db->from('cake_suspended_items_taxes');
