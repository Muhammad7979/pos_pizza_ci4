(function ($) {
    ...
    $.fn.bootstrapTable.locales['<?php echo $appData['language']; ?>'] = {
		...
		formatNoMatches: function () {
			return "<?php 
			if(preg_match('(customers|suppliers|vendors|employees|counters)', $controller_name)){
				echo lang(preg_match('(customers|suppliers|vendors|employees|counters)', $controller_name) ?
				'common_lang.common_no_persons_to_display' :  $controller_name.'_lang.'.$controller_name . '_no_' . $controller_name .'_to_display');
			}elseif(preg_match('(items|raw_items|item_kits|warehouses|stores|raw_orders|counter_orders|giftcards|store_orders)', $controller_name)){
				echo lang(preg_match('(items|raw_items|item_kits|warehouses|stores|raw_orders|counter_orders|giftcards|store_orders)', $controller_name) ?
				'common_lang.common_no_data_to_display' :  $controller_name.'_lang.'.$controller_name . '_no_' . $controller_name .'_to_display');
			}else{
				echo lang('common_lang.common_no_data_to_display');
			}
