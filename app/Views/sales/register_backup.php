...
<style>
    #buttons_sale, #payment_details, #suspended_sales_table, .sales_table_100 {
        margin: 14px 21px;
        width: auto
    }
...
<div class='alert alert-dismissible alert-warning print_hide'
     id="errorMsg" style="display: none;">
    <b></b>
...
<div class="top_bar print_hide">
    <div class="inner_block">
        ...
        <div class="dropdown_col">
            ...
            <?php echo form_dropdown('mode', $modes, $mode, array('onchange' => "$('#mode_form').submit();",
        ...
        <div class="dropdown_col">
            ...
            <?php echo form_dropdown('type', $types, $type, array('onchange' => "$('#type_form').submit();",
                'class' => 'dropdown selectpicker btn_register_mode show-menu-arrow',
                'style' => 'background-color:#e7e7e7; border:#e7e7e7; color:#484848;',
        ...
        <div class="suspended_block">
            ...
            if ($this->Employee->has_grant('reports_sales', $this->session->userdata('person_id'))) {
                ...
                <?php echo anchor($controller_name . "/manage", '<span class="glyphicon glyphicon-list-alt">&nbsp;</span>' . $this->lang->line('sales_takings'),
                    array('id' => 'sales_takings_button', 'title' => $this->lang->line('sales_takings'))); ?>

                <?php
            }
            ?>

            <button id='show_suspended_sales_button'
                    data-href='<?php echo site_url($controller_name . "/suspendedWithItems"); ?>'
                    title='<?php echo $this->lang->line('sales_suspended_sales'); ?>'>
                <span
                        class="glyphicon glyphicon-align-justify">&nbsp;</span><?php echo $this->lang->line('sales_suspended_sales'); ?>
            </button>

            <button id='show_shortcuts_button'
                    data-href='<?php echo site_url($controller_name . "/shortcuts"); ?>'
                    title='View Shortcuts'>
                <span
                        class="glyphicon glyphicon-align-justify">&nbsp;</span>Shortcuts</button>
        </div>
    ...
    <div class="search_outer">
        <div class="search_block">
            ...
            <input type="text" name="item" value="" id="item"
                   class="search_input ui-autocomplete-input" tabindex="1" autocomplete="off">
            <!--            placeholder="Start Typing item’s name or scan barcode .."-->
...
<div id="register_content">
    <?php $this->load->view('sales/register_partial'); ?>
</div>
...
//                        console.log(cartItem.find('input[name=price]').val());
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                $('#cartItem' + id).effect('highlight',{color:'#18bc9c'},3000);
        ...
        //             var oId = $(this).val();
        //             alert(oId);
        //             $.get( "<?php //echo base_url('sales/getId/'); ?>"+oId)
        //               .done(function( data ) {
        //                 obj = JSON.parse(data);
            ...
            if ($("#payment_types").val() === "<?php echo $this->lang->line('sales_giftcard'); ?>") {
                if(total_price > 0){
                    if($('#amount_tendered').val()==''){
                        $('#error_message_box').html('<li><label id="item_number-error" class="has-error" style="display: inline-block;" for="item_number"><?php echo $this->lang->line('sales_amount_required'); ?></label></li>').fadeIn();
                    }else if(!$.isNumeric($('#amount_tendered').val())){
                        $('#error_message_box').html('<li><label id="item_number-error" class="has-error" style="display: inline-block;" for="item_number"><?php echo $this->lang->line('sales_amount_required_numeric'); ?></label></li>').fadeIn();
                    }else if($('#amount_giftcard_tendered').val().length<3){
                        $('#error_message_box').html('<li><label id="item_number-error" class="has-error" style="display: inline-block;" for="item_number"><?php echo $this->lang->line('sales_giftcard_not_valid'); ?></label></li>').fadeIn();
                    }else if($('#amount_giftcard_tendered').val()==''){
                        $('#error_message_box').html('<li><label id="item_number-error" class="has-error" style="display: inline-block;" for="item_number"><?php echo $this->lang->line('sales_giftcard_required'); ?></label></li>').fadeIn();val
                    }else if(($('#amount_giftcard_tendered').data("value")<=0 || $('#amount_giftcard_tendered').data("value")=='') && $('#amount_giftcard_tendered').data("voucher")=='' ){
                        $('#error_message_box').html('<li><label id="item_number-error" class="has-error" style="display: inline-block;" for="item_number"><?php echo $this->lang->line('sales_giftcard_expire'); ?></label></li>').fadeIn();
                        ...
                        $('#error_message_box').html('<li><label id="item_number-error" class="has-error" style="display: inline-block;" for="item_number"><?php echo $this->lang->line('sales_giftcard_low_limit'); ?>'+giftMinVal+'</label></li>').fadeIn(); 
                    }else{
                        $('#add_payment_form').submit();
                    }
                }else{
                    $('#error_message_box').html('<li><label id="item_number-error" class="has-error" style="display: inline-block;" for="item_number"><?php echo $this->lang->line('sales_giftcard_low_limit'); ?></label></li>').fadeIn();
                }
            }else{
                $('#add_payment_form').submit();
            }
                    ...
                    $.get( "<?php echo base_url('sales/giftcard_value/'); ?>"+giftcard_num)
                      .done(function( data ) {
                        ...
                        if(obj.value==0){
                            ...
                            $('#error_message_box').html('<li><label id="item_number-error" class="has-success" style="display: inline-block;" for="item_number"><?php echo $this->lang->line('sales_giftcard_success'); ?></label></li>').fadeIn();
                            ...
                            if (total_price>obj.value) {
                                $("#sales_payment_type").attr( "style", "display: block !important;" );
                            }

                        }else{
                           $('#error_message_box').html('<li><label id="item_number-error" class="has-error" style="display: inline-block;" for="item_number"><?php echo $this->lang->line('sales_giftcard_low_limit'); ?>'+obj.value+'</label></li>').fadeIn(); 
...
if (isset($direct) && $direct) {
    redirect(base_url('sales/complete'));
}
?>
    ...
    <script>
        $(document).ready(function () {
            <?php if ($this->router->fetch_method() == "complete"): ?>
            printdoc();
