					...
					<table>
						<thead>
							<tr class="header">
								...
								<!-- <th>Price</th> -->
								<th>Status</th>
								<th class="details-title">Details</th>
								<!-- <th></th> -->
							</tr>
						</thead>
						...
						?>							
							<tr class="order_row_<?php echo $value->order_id ?>">
								...
								<td><?php echo parse_decimals($value->order_quantity) ?></td>
									...
									<table>
										<tbody>
											...
											<tr>
												<td>
													<?php echo $item->name ?>
													<?php if($item->is_half=='Yes'){ echo " + ".$item->flavor2; } ?> <span>|</span> <?php echo $item->size ?> <span>|</span> <?php echo parse_decimals($item->quantity) ?> <span>|</span> <?php echo $item->layer ?> <span>|</span> <?php echo $item->dough ?>  <br>
    ...
    <script type="text/javascript">        
		...
		function trigger_keypress(keyCode) {
		    ...
		    if(id>0){
            	...
            	$.ajax({
	                type: "POST",
	                url: "<?php echo site_url('pizza_orders_list/update_status/') ?>"+id+'/'+microtime+'/'+status,
	                beforeSend: function() {
	                    // setting a loader
	                    $("#overlay").fadeIn(300);　
	                },
		    ...
		    //window.location.href = "<?php //echo site_url('pizza_orders_list/update_status/') ?>"+id+'/'+microtime+'/'+status;
		}

		function getLatestOrders(){
			$.ajax({
                type: "POST",
                url: "<?php echo site_url('pizza_orders_list/getLatestOrders/') ?>",
                beforeSend: function() {
                    // setting a loader
                    $("#overlay").fadeIn(300);　
                },
                ...
                {   
                	...
                	$.each(data.pizza_items, function(i, idata) {
                		...
                		$('#append_new_order'+idata.store_id).append('<tr class="order_row_'+idata.order_id+'"><input type="hidden" name ="microtime" value="'+idata.microtime+'" ><input type="hidden" name ="order_id" value="'+idata.order_id+'" ></td><td>'+idata.order_number+'</td><td>'+Math.round(idata.order_quantity)+'</td><td>'+idata.order_status+'</td><td class="details"><table><tbody>'+order_items+'</tbody></table></td></tr>');
                		}
                	});
